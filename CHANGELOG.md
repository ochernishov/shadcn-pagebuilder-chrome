...
## 2026-08-25 — CI переехал на собственный раннер

Минуты GitHub Actions на приватных репозиториях платные: замер 25.08.2026 по аккаунту — 1608 прогонов за 30 дней в 17 репозиториях, около $30 сверх подписки Pro за месяц. Прогон на своей машине не тарифицируется.

### Changed

- `runs-on: ubuntu-latest` → `runs-on: [self-hosted, cosinn]` во всех workflow. Машина `cos-runner` — 135.106.196.218, 4 vCPU / 8 ГБ / 80 ГБ, Ubuntu 24.04, заведена по §6 скилла `/servers`.
- Метка `cosinn` обязательна: в части репозиториев Cosinn меткой `self-hosted` помечены раннеры деплоя на боевых серверах — без своей метки сборка уехала бы на прод. Раннеры деплоя этого репозитория, если они есть, оставлены как были.


## 2026-08-24 — Триггер CI ловил PR в любую ветку (#13)

### Fixed
- ** без фильтра веток** запускал прогон на PR в любую ветку. С
  новой схемой работы (крупная фича = ветка эпика , фазы вливаются в неё
  PR-ами, в  уходит один PR и один прогон) каждый фазовый PR давал здесь
  лишний прогон. Добавлен ; заодно появился 
  для ручного перезапуска.
- Триггер  **оставлен намеренно**, хотя в приватных репозиториях
  контура он снят как дубль: репозиторий публичный, минуты Actions на него не
  тратятся, а бейдж CI в README без push-прогонов показывал бы «no status».
