...
- Create an isolated six-digit package such as `Collector/482731/` for every finished page.
- Install the shared global `shadcn-collector` skill for both Codex and Claude Code.
...
1. Download `Shadcn-Page-Collector-v0.7.0.zip` from the latest GitHub Release.
...
The installer copies the Native Messaging host into `~/Library/Application Support/Shadcn Page Collector/` and installs `shadcn-collector` into both `~/.codex/skills/` and `~/.claude/skills/`. The downloaded package can be removed after Chrome has loaded the extension. Node.js 20+ is currently required by the local export host.
...
11. Click **Finish page**. Collector creates a new six-digit package, shows its number, and provides a **Copy number** action.
12. Open the chosen project in Codex or Claude Code and invoke `shadcn-collector` with that number.

When a project folder is selected, exports are written to:

```text
<Project>/Collector/<six-digit-number>/
```

Without a selected project folder, the fallback location is:

```text
~/Documents/Page Collector/<Project>/Collector/<six-digit-number>/
├── PAGE_SPEC.md
├── page-spec.json
└── references/
    ├── 01-hero1.png
...
Every click on **Finish page** creates a new folder and never mixes files from different collections. The number is also written into `PAGE_SPEC.md` and `page-spec.json`.

## Use with Codex or Claude Code

Restart the agent after the first installation so it discovers the global skill. Open the same project directory that was selected in Page Collector, then use one of these forms:
...
The skill resolves only the exact local path `Collector/482731/`, validates the ID in `page-spec.json`, reads `PAGE_SPEC.md` and every PNG in `references/`, and then implements the page. It does not scan other projects or the home directory.

For each saved item, the implementation agent decides whether an authorized official installer is available. Without source access, it creates an original implementation from the screenshots and notes without bypassing access controls.
...
| `COLLECTOR_SKILL_NAME` | Shared skill folder name for coding agents |
| `CODEX_SKILLS_DIRECTORY` | Global Codex skill installation directory |
| `CLAUDE_SKILLS_DIRECTORY` | Global Claude Code skill installation directory |
